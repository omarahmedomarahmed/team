...
export default async function Dashboard() {
  ...
  const stats = [
    { label: "Services", value: services, href: "/god/services", icon: "settings" },
    { label: "Portfolio", value: projects, href: "/god/projects", icon: "presentation" },
    { label: "Team", value: team, href: "/god/team", icon: "briefcase" },
    { label: "Testimonials", value: testimonials, href: "/god/testimonials", icon: "star" },
    { label: "Blog posts", value: posts, href: "/god/posts", icon: "pen-tool" },
    { label: "Leads", value: leadsTotal, href: "/god/leads", icon: "mail" },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      {newLeads > 0 ? (
        <Link href="/god/leads" className="card card-hover p-4 flex items-center gap-3">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/god/leads" className="text-sm text-brand">View all</Link>
        </div>
        <div className="card divide-y divide-line">
          {recentLeads.length === 0 ? (
            ...
            recentLeads.map((l) => (
              <Link key={l.id} href={`/god/leads/${l.id}`} className="flex items-center gap-4 p-4 hover:bg-[var(--card)] transition-colors">
