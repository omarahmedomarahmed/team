 ...
 * counts (3 / 3 / 6 / 4 / 8) are enforced here so the rhythm never breaks.
 */
export function CaseStudy({ exp }: { exp: Experience }) {
  ...
  return (
    <article className="container-x section-pad">
      <Link href="/#timeline" className="inline-flex items-center gap-1.5 text-sm text-muted hover:text-fg">
        <Icon name="arrow-left" size={15} /> Back to timeline
      </Link>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            {(gallery.length ? gallery : [null, null]).map((g, i) => (
              <div key={i} className="media-frame aspect-[4/3]">
                {g ? <img src={g} alt="" className="h-full w-full object-cover" /> : <Placeholder label="Showcase" />}
              </div>
            ))}
          </div>
        </Reveal>
